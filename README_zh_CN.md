...
#### 方式一：自动菜单（推荐）
1. 创建一个新的引用块（输入 `>` 然后按空格）
2. 会自动弹出命令菜单
3. 使用方向键或鼠标选择你想要的 Callout 类型
4. 按 Enter 或点击确认

#### 方式二：手动命令
在引用块的开头输入 Callout 命令：
...
|---------|---------|---------|---------|
| `@info` | `@信息` | 信息说明 | 一般性信息 |
| `@concept` | `@概念` | 概念解释 | 概念性解释 |
| `@example` | `@示例` | 示例演示 | 示例演示 |
| `@tip` | `@技巧` | 使用技巧 | 使用技巧 |
| `@best-practice` | `@最佳实践` | 最佳实践 | 最佳实践 |
| `@tradeoff` | `@权衡取舍` | 权衡取舍 | 权衡取舍 |
| `@deep-dive` | `@深水区` | 深水区 | 深入探讨 |
| `@comparison` | `@对比` | 对比分析 | 对比分析 |
| `@summary` | `@总结` | 章节总结 | 章节总结 |
| `@pitfall` | `@陷阱` | 常见陷阱 | 常见陷阱 |
| `@highlight` | `@亮点` | 重要亮点 | 重要亮点 |

### 交互功能

- **切换类型**：点击 Callout 标题左侧的图标区域可以切换类型
- **折叠/展开**：点击标题文本区域（右侧）可以折叠或展开内容
- **快速访问**：使用命令菜单可以快速选择 Callout 类型
