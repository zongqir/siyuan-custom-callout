...
- 完全使用原生 `.callout[data-subtype]` 进行样式渲染，支持任意自定义子类型
- 别名匹配：`type` + 中文命令（去方括号）+ `displayName` 均可命中同一套样式
- 按类型渲染图标：图标与 `data-subtype` 同步，原生类型亦有兜底图标
- 更高 CSS 特异性 + 大小写不敏感匹配 `[i]`，对主题更友好
- 稳定性：修复创建/确认时可能出现的卡顿与光标丢失问题
- 仅保留 V2 架构：彻底移除 V1 路径
...
- 🧩 **原生 data-subtype** - 任意 `data-subtype` 值均可被样式化（含自定义）
- 🖼️ **按类型图标** - 图标自动匹配解析后的 `data-subtype`
- 📐 **拖拽调整** - 可调整宽度和高度
- 🔒 **非侵入式** - 不修改笔记内容，仅渲染样式
